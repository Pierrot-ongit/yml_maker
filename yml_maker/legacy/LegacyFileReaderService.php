...
class LegacyFileReaderService {
  ...
  public function loadFileContent($fid, $files_infos){
    ...
    $this->file = file($this->fullNamePath);
  ...
  // TODO : Créer des Lignes ID et des Parents ID.
  ...
  public function testCrash(){
    ...
    // TODO : Une fonction qui vérifie que la réecriture du fichier ne fait pas planter :
    // 1°) Le site tout entier.
    // 2°) Les pages-url que lui on aura fourni dans un champs adapté au dessus de la table.
    // UNe option intéressante à rajouter dans les settings du module.
  ...
  public function writeFile($values){
    ...
    // TODO :
   // On va devoir recréer les lines, avec l'indentation., L'inverse du prepareFileContent.
    // Comment déterminer l'ident en fonction de la weight et du plid ?
    ...
    foreach ($values as $key=>$value){
      ...
      // Si le plid est inférieur ou égale à un, alors la ligne n'a aucun parent. Donc pas d'identation.
      if($plid == 1 || $plid < 1){
        $file[$lid]['identSize'] = 0;

      }

      else{
        // Et sinon elle a forcement une identation. Du coup on prend l'identation de son parent (qui existera forcement), et on y ajoute 2.
        $parent = $file[$plid];
        $parentIdentSize = $parent['identSize'];
        $file[$lid]['identSize'] = $parentIdentSize + 2;
